...
interface FailedTransaction {
  index: number;
  address: string;
  stage: "prepare" | "approval" | "preflight" | "send" | "receipt";
  error: string;
  to?: string;
  selector?: string;
  value?: string;
  ...
  while (true) {
    ...
    try {
      for (const approval of trade.approvals ?? []) {
        const approvalGas = approval.gas ?? defaultGasLimit;
      ...
      failures.push({ ...context, stage: "approval", error: describeError(error) });
      continue;
    }
    try {
